...
```sh
git clone https://github.com/mshauneu/avalanche-network-runner.git
```
...
Assumes you have the latest AvalancheGo binaries at `~/Opt/avalanchego/avalanchego`. For instructions on setting up AvalancheGo, see [here.](https://github.com/ava-labs/avalanchego)

```sh
go build -o ~/Bin/ava examples/local/fivenodenetwork/main.go
```
...
This run another example at `examples/local/fivenodenetwork/main.go`, which just starts a local five node network, waits for the nodes to become healthy, and then runs until the user provides a SIGINT or SIGTERM.
